...
>>> split-common-prefix 左䢼分 "返回􏾴：移除两个􏿴左起的相同元素后剩余的两个􏿴"
>>> drop-common-prefix 左􏾶分 "返回􏾴：两个􏿴左起的相同元素组成的􏿴，原两个􏿴中剩余部分组成的􏿴" ;; ;;􏾺异项 􏾺异相 共异/􏾺
...
>>> filter-map 𮞑而􏾽 "(􏾽 (入 (x) x) (𮞑 proc '(...)))"
>>> append-map 𮞑而􏿝 "(􏿝 (𮞑 proc '(...)))"
