...
>>> exact-integer? 􏻲? "准确整数? （各取偏旁得造）" ; 􏻲 􏻲 􏺢 􏺢 􏺢
...
>>> positive? 􏻛? "正数（各取偏旁得造）" ;􏽌 􏻨 􏻛
>>> negative? 􏻚? "负数（各取偏旁得造）" ;􏽋 􏻩 􏻔 􏻚
>>> even? 􏺦? "偶数（阳整数）" ;𥻑 􏻣 􏻖 􏻘 􏺷 􏺷
>>> odd? 􏺧? "奇数（阴整数）" ; 􏻢 􏻕 𥺿 􏺶 􏺶
>>> exact? 􏻧? "准确数（各取偏旁得造；所有数字要么是准确数，要么不是）" ; 􏻗 􏻧 􏺸
>>> inexact? 􏺟? "非准确数（各取偏旁得造；非准确数是用浮点数实现的）" ;非􏻧 􏻜
>>> inexact->exact 􏻧化 @elem{@racket[􏻜]转化为@racket[􏻧]} ;􏻧化 􏺸化
