...
>>> fixnum? 𠤻? @elem{固定数（@racket[􏻲]的一种具体实现）} ;􏻥
>>> flonum? 釆? "浮点数（各取偏旁得造）" ;粰 釆 􏻪
>>> double-flonum? 􏻝? @elem{双倍精度浮点数（仿@litchar{􏻞}，且占用64位内存而得造）} ; 双釆?
...
>>> positive? 􏻛? "正数（各取偏旁得造）" ;􏽌 􏻨
>>> negative? 􏻚? "负数（各取偏旁得造）" ;􏽋 􏻩 􏻔
>>> even? 􏻘? "偶数（各取偏旁得造）" ;𥻑 􏻣 􏻖
>>> odd? 􏻙? "奇数（各取偏旁得造）" ; 􏻢 􏻕
>>> exact? 􏻧? "准确数（各取偏旁得造，所有数字要么是准确数，要么不是）" ; 􏻗
