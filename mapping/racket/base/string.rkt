...
(define data
  '([string? 诗?]
    [string 诗 "创建一个可以内容改变的字符串数据结构。"] ;; TODO: create all the procedures started with 易 to 坚
    [make-string 复诗 "创建一个可以内容改变的字符串数据结构，其内容为数个重复相同的字。"]
    [string->immutable-string 诗->坚诗 "把诗转变为坚诗。"]
    [string-length 诗长]
    [string-ref 诗引]
    [string-set! 诗改!]
    [substring 摘诗 "索引摘录诗的N到F位之间的字，并生成一个易诗。"] ;; 摘
