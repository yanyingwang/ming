...
>>> string-set! 拘 @elem{直接修改原始数据第N处字（从0算起）为某字} ;句􏾩 敂!
>>> substring 邭 @elem{返回句第N到M处内容组成的@racket[􏽀]} ;句􏾝
>>> string-copy 􏽀化 @elem{把@elem{􏽁}转化为@elem{􏽀}}
>>> string-copy! 拘/以邭 @elem{从第N处字（从0算起）开始做覆盖（以另一个句的@racket[句􏾝]结果为内容）} ;句􏾩/以􏾝
>>> string-fill! 句􏼿 "把句的内容全部变成某一个重复的字" ; 句㙏! 句复!
>>> string-append 􏼱 @elem{把很多句连成一个@racket[􏽀]} ; 贯句
>>> string-append-immutable 􏼯 @elem{把很多句连成一个@racket[􏽁]}

>>> string->list 􏼳 @elem{把@racket[句]转化为@racket[􏿴]（由@racket[字]组成）} ;句化􏿴
>>> list->string 􏼲 @elem{把@racket[􏿴]（由@racket[字]组成）转化为@racket[句]} ; 􏿴化句
