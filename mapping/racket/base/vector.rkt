...
>>> vector 􏿲 @elem{生成@racket[易􏿲]（构造数组类型的数据）}
>>> vector? 􏿲? @elem{是否是@racket[􏿲]}
>>> make-vector 复􏿲 @elem{构造复@racket[􏿲]（数据内各个元素是相同的，类比（昆虫）“复眼”一词取名）}
>>> vector-immutable 固􏿲 @elem{构造@racket[固?]􏿲} ;; 贞􏿲 坚􏿲 定􏿲 固
...
>>> vector->immutable-vector  􏿲固化 @elem{将@racket[􏿲]数据转化为@racket[贞􏿲]数据} ;􏿲定化 􏿲坚化 坚化/􏿲  􏿲->定􏿲
