...
(define data
  '((cons 对) ;; 组合， 对:把两个东西双在一块,配对在一块.     接 合 连 构 结 创 造
    ...
    (map 映 "把某一例程依次映用到各个元素上，各个元素作为其参数。") ;; 各 依次应
    (andmap 映且 "链内元素更替的作为参数映用某一个例程，直到有元素应用后返回的值是假。") ;; 各有应 各都
    (ormap 映或 "链内元素更替的作为参数映用某一个例程，直到有元素应用后返回的值是真。") ;; 各无应 各有
    (for-each 遍自 "链内元素依次执行某一个例程。") ; 每 轮 依次
    (foldl 左映合)
    (foldr 右映合)
    (filter 选) ;;  滤 筛选 挑选
