...
>>> andmap 𮞑都为 "􏿴中各个元素全都是...的吗？" ;; 各有应 各都 映且
>>> ormap 𮞑中有 "􏿴中各个元素中有是...的吗？" ;; 各无应 各有 映或

>>> for-each 巡 "酒席上给全座依次斟酒一遍称为巡杯（酒过三巡），这里意思是􏿴内元素依次执行某一个例程。" ; 轮着 每 轮 依次 遍自 遍执 行 循序

>>> foldl 𮞑左参与 "􏿴中各个元素依次作为某例程的最左遍的参数，和本有剩余的参数一块依次反复的应用到某例程上。"  ;; 各左参
>>> foldr 𮞑右参与 "􏿴中各个元素依次作为某例程的最右遍的参数，和本有剩余的参数一块依次反复的应用到某例程上。"
