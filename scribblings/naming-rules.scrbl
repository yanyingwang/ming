...
下面让我们把以上所讲内容汇总成表格，来观察一下编程语言各个概念之间的关联关系是如何被用汉字造字法创造出来的字符所体现出来的：

@tabular[#:style 'boxed
         #:column-properties '(left left)
         #:row-properties '(bottom-border ())
         (list
         (list @bold{English Word}  @bold{中文字符}   @bold{数据结构示例} @bold{数据结构创建示例})
         ...
         (list @racket[class] "䄌" "" "")
         (list "----------" "---" "----------------------------" "---------------------------------------")
         (list @racket[length]  "度"       "使用示例："  @racket[(度 '(a b c d e)) ==> 5])
         (list @racket[vector-length] "􏿺" "使用示例："  @code|{(􏿺 '矢(a b c d e)) ==> 5}|)
         (list @racket[string-length] "𬤏" "使用示例："  @racket[(𬤏 "abcde") ==> 5])
         )]


如果我们把@litchar{又}看成是编程中数据的概念抽象的话，通过以上例子，我们就可以明白要如何用汉字的造字法来的表达LISP语言里面的固有概念。并且我们能看到，较之英文，汉字的表达是更能体现出概念之间的逻辑关系的。

另外值得注意的是，我们如果把汉字的这个字符创造法用在其他的编程语言上恐怕未必能达到以上这样的效果，这主要因为LISP编程语言的极简主义特色：把一些简单的概念层层堆叠以组建表达复杂的概念。这个特色在我看来，和中国上古时期易经的“太极生两仪两仪生四象四象生八卦”，亦或是春秋战国时期道家老庄哲学的“道生一，一生二，二生三，三生万物”有异曲同工之妙。然后再反观中国汉字的造字法，会感觉这三者都遵从着同一理念。
